...
import numpy as np
import os
from numba import jit, vectorize, int32, char, int64
from tkinter import StringVar

# ASCII chars used
ASCII = ['  ', '..', '<<', 'cc', '77', '33', 'xx', 'ee', 'kk', '##', '■■']
ASCII_INT = np.asarray([ord(elem[0]) for elem in ASCII])
...
def assign_ascii(number):
    index = number//25
    return ASCII_INT[index]


if __name__ == '__main__':
    print(assign_ascii(np.zeros(5, dtype='int32')+70))
